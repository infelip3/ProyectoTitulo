...
import { getTypes, getLevels, getSpecies, getCases, generatePlusReport } from '@/utils/cases.js';
...
const handleLoadChart = async () => {
  try {
    ...
    const chartData = cases
      .reduce((carry, caseItem) => {
        const foundCase = carry.find((item) => item.commune === caseItem.commune);
        if (foundCase) {
          foundCase.quantity += 1;
        } else {
          carry.push({
            commune: caseItem.commune,
            quantity: 1,
          });
        }
    ...
    const labels = chartData.map((item) => item.commune);
    const datasets = [
      {
        label: '# de casos',
